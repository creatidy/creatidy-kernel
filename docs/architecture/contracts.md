...
| Forge | Resolve opaque repository/change/revision/check references; normalize domain receipts; create/update only authorized effects; observe current remote state and capability-gate conditional mutations. PR creation uses an exact accepted-commit snapshot head with enforced write-once isolation through creation; its base authority is repository plus target branch, with observed base SHA retained as evidence. If snapshot isolation is unavailable, PR creation is unsupported. | Forgejo and fake run the same suite; pagination, replay, stale accepted head, moving PR base tip without changing base identity, inaccessible versus absent, conflicting idempotency key, uncertain merge, missing exact-head check. No `gh` subprocess or Forgejo payload in core. |
