...
| Forgejo CI, mirror and release authority split | BORROW DESIGN | Reuse `ci`/`check`, develop PR validation, pinned actions, no secrets in PR workers, exact-artifact release discipline. Do not copy the Windows/OCI/PyPI release pipeline into an empty foundation. |
